...
from network.model import Downsample, Upsample, InputProj, OutputProj, BasicPanoformerLayer
from network.equisamplingpoint import genSamplingPattern

class PanoformerMult(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
        ...
        # the encoder layer for input process..
        ...
        # the output layer for output_final process....
        ...
        # the output layer for output_2x process....
        self.output_proj_2x = OutputProj(in_channel=4 * embed_dim, out_channel=1, kernel_size=3, stride=1, input_resolution=(img_size//2, img_size))

        # the output layer for output_4x process....
        self.output_proj_4x = OutputProj(in_channel=8 * embed_dim, out_channel=1, kernel_size=3, stride=1, input_resolution=(img_size//4, img_size//2))

        # the output layer for output_8x process....
        self.output_proj_8x = OutputProj(in_channel=16* embed_dim, out_channel=1, kernel_size=3, stride=1, input_resolution=(img_size//8, img_size//4))
    ...
    def forward(self, x):
        ...
        y2x = self.output_proj_2x(deconv2)
        y4x = self.output_proj_4x(deconv1)
        y8x = self.output_proj_8x(deconv0)
        outputs = {}
        outputs["pred_depth"]   = y
        outputs["pred_depth2x"] = y2x
        outputs["pred_depth4x"] = y4x
        outputs["pred_depth8x"] = y8x
        return outputs
